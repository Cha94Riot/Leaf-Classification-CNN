...
def main():
    ...
    train_data_dir = 'C:\\Users\\Adam\\PycharmProjects\\CNN\\altDS\\Train'
    test_dat_dir = 'C:\\Users\\Adam\\PycharmProjects\\CNN\\altDS\\Test'
    ...
    # Input = 256 x 256 x 3 = 196608
    model.add(Conv2D(8, (3, 3), strides=(2, 2), name='Conv1-2'))
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    # Input = 127 x 127 x 8 = 129032
    model.add(Conv2D(16, (3, 3), strides=(2, 2), name='Conv2-2'))
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    # Input = 62 x 62 x 16 = 61504
    model.add(Conv2D(32, (3, 3), strides=(2, 2), name='Conv3-2'))
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    # Input = 30 x 30 x 32 = 28800
    model.add(Conv2D(64, (3, 3), strides=(2, 2), name='Conv4-2'))
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    # Input = 14 x 14 x 64 = 12544
    model.add(Conv2D(128, (3, 3), strides=(2, 2), name='Conv5-2'))
    model.add(BatchNormalization())
    model.add(Activation("relu"))
    ...
    model.compile(loss=keras.losses.categorical_crossentropy, optimizer=keras.optimizers.Adam(), metrics=['acc', 'mae'])
    history = model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=epochs, batch_size=200)

    model.save('altDSModel.h5')
    model.save_weights('altDSWeights.h5')
